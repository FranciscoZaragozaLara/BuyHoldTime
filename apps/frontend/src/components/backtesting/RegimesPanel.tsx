              ...
              {paginated.map((r:any, idx:number)=>{
                ...
                return (
                  <tr key={r.date} ref={isLast? observerRef : null} className="hover:bg-slate-800/40 border-t border-slate-800">
                    ...
                    <td className={`p-2 text-right font-mono font-semibold border rounded ${mgZColor} cursor-help`} onMouseEnter={(e)=>{ const n= mgZ!=null ? (detRow.r2.active ? (<div className="space-y-1.5 text-left uppercase tracking-wide leading-tight"><div className="font-semibold text-sky-400">ARMADO Fase 1 — Z ACTUAL <span className="text-sky-400">{mgZ.toFixed(2)}</span></div><div className="font-mono text-[11px]">Z=(<span className="text-slate-300">{mg?.toFixed(2)}</span>−<span className="text-slate-400">{detRow.r2.mean?.toFixed(2) ?? '—'}</span>)/<span className="text-slate-400">{detRow.r2.std?.toFixed(2) ?? '—'}</span>=<span className="text-sky-400">{mgZ.toFixed(2)}</span> <span className="text-sky-400">— ARMADO POR Z&gt;2.0 en {(detRow.r2 as any).armedMonth ?? 'ÚLTIMOS 6M'} — PERSISTE HASTA Z&lt;0</span></div></div>) : (<div className="space-y-1.5 text-left uppercase tracking-wide leading-tight"><div className="font-semibold text-emerald-400">NO ARMADO — Z ACTUAL <span className="text-emerald-400">{mgZ.toFixed(2)}</span></div><div className="font-mono text-[11px]">Z=(<span className="text-slate-300">{mg?.toFixed(2)}</span>−<span className="text-slate-400">{detRow.r2.mean?.toFixed(2) ?? '—'}</span>)/<span className="text-slate-400">{detRow.r2.std?.toFixed(2) ?? '—'}</span>=<span className="text-emerald-400">{mgZ.toFixed(2)}</span> <span className="text-slate-500">· ningún Z&gt;2.0 en 6M</span></div><div className="text-[11px]"><span className="text-emerald-400">ESTADO:</span> <span className="text-emerald-300">DESARMADO — SIN PELIGRO SISTÉMICO</span></div></div>)) : (<div className="text-slate-400">Z SIN VENTANA</div>); const r=(e.currentTarget as HTMLElement).getBoundingClientRect(); setTableTip({x:r.left+r.width/2, y:r.top, node:n}); }} onMouseLeave={()=>setTableTip(null)} onMouseMove={(e)=>{ const n= mgZ!=null ? (detRow.r2.active ? (<div className="text-sky-400">ARMADO {mgZ.toFixed(2)}</div>) : (<div className="text-emerald-400">NO ARMADO {mgZ.toFixed(2)}</div>)) : (<div>—</div>); const r=(e.currentTarget as HTMLElement).getBoundingClientRect(); setTableTip({x:r.left+r.width/2, y:r.top, node:n}); }}>{mgZ!=null? mgZ.toFixed(2):'—'}</td>
