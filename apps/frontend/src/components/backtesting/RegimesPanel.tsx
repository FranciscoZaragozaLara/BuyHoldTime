...
export function RegimesPanel() {
  ...
  function getRegimeDetails(d:string, capeVal:number|null, mean3yVal:number|null){
    ...
    // R1 — Condición Estructural 4 fases: CAPE>1.20 AND Core CPI>1.15 (SMA12M)
    ...
    let r1CoreCpiVal:number|null=getClosestPrice(marketHistory['CPILFESL'], d);
    // SMA12M Core CPI mensual (12 meses)
    if(r1CoreCpiVal!=null){
      const cpiMonthlyMap = new Map<string, number>();
      for(const dd of allCapeDates){ const mk=dd.slice(0,7); const v=marketHistory['CPILFESL']?.get(dd) ?? getClosestPrice(marketHistory['CPILFESL'], dd); if(v!=null) cpiMonthlyMap.set(mk, v); }
      const cpiKeys = Array.from(cpiMonthlyMap.keys()).sort();
      const curMk = d.slice(0,7);
      let cIdx = cpiKeys.indexOf(curMk); if(cIdx===-1){ let b=-1; for(let i=0;i<cpiKeys.length;i++) if(cpiKeys[i]<=curMk) b=i; cIdx=b; }
      if(cIdx>=0){ const sKeys=cpiKeys.slice(Math.max(0,cIdx-11),cIdx+1); const vals=sKeys.map(k=>cpiMonthlyMap.get(k)).filter((v):v is number=>v!=null); if(vals.length>=6){ r1CpiSma12=getSMA(vals); } }
      r1Cpi=r1CoreCpiVal;
    }
    if(capeVal!=null && mean3yVal!=null){ r1Thr=mean3yVal*1.20; r1Ratio=capeVal/mean3yVal; const capeOk=capeVal > r1Thr; const cpiOk = r1Cpi!=null && r1CpiSma12!=null ? r1Cpi > r1CpiSma12*1.15 : false; r1Active=capeOk && cpiOk; r1Armed=r1Active; }
      ...
      if(mIdx>=0){
        ...
        for(let k=0;k<6;k++){
          ...
          if(mean2!=null && std2!=null && std2>0){
            ...
            if(z2>2.0){ found=true; r2ArmedMonth=mKey2; const lastForMonth = [...allCapeDates].reverse().find(dd=> dd.startsWith(mKey2)) ?? mKey2; r2ArmedDate = lastForMonth; break; }
          }
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-slate-900 z-10">
              <tr className="text-slate-400">
                ...
                <th className="p-2 text-center" title="Múltiplos Altos 4 Fases — F0 Armado: CAPE>1.20 & Core CPI>1.15 (12M) — G1: Fed>0 o 10Y>1.10 (30%) — G2: Fed≥+0.50 & CPI>1.30 (70%) — G3: 3M caídas o Pausa2M (35%) — G4: CPI<1.05 o Fed<0 + SMA50 (65%) — Sanación CPI<1.05+SMA50">📈</th>
                ...
                return (
                  <tr key={r.date} ref={isLast? observerRef : null} className="hover:bg-slate-800/40 border-t border-slate-800">
                    ...
                    {(() => {
                      ...
                      return (<>
                    <td className="p-2 text-center"><span className={icon(d1.active,'bg-amber-500/20 text-amber-300 border-amber-500/30','bg-slate-800 text-slate-500 border-slate-700')} onMouseEnter={(e)=>{ const c=d1.cape, s=d1.sma, th=d1.thr; const isRed = d1.active; const cpi=(d1 as any).cpi; const cpiSma=(d1 as any).cpiSma12; const cpiThr=cpiSma!=null?cpiSma*1.15:null; const capeOk=c!=null&&th!=null&&c>th; const cpiOk=cpi!=null&&cpiSma!=null&&cpi>cpiThr!; const node=isRed?(<div className="space-y-1.5 text-left uppercase tracking-wide leading-tight text-slate-200"><div className="font-semibold text-red-400">📈 MÚLTIPLOS ALTOS ON ✓ — BOSQUE SECO (F0 ARMADO) <span className="text-red-400" style={{fontSize:'11px'}}>· +20% CAPE & +15% CPI</span></div><div>CAPE <span className="text-red-300 font-black">{fmt(c)}</span> <span className="text-red-400">&gt;</span> UMBRAL <span className="text-slate-200 font-bold">{fmt(th)}</span> <span className="text-slate-400">(=SMA</span> <span className="text-slate-300">{fmt(s)}</span><span className="text-slate-400">×1.20)</span> <span className="text-slate-400">·</span> CORE CPI <span className="text-red-300 font-bold">{fmt(cpi)}</span> <span className="text-red-400">&gt;</span> <span className="text-slate-200">{fmt(cpiThr)}</span> <span className="text-slate-400">(×1.15)</span></div><div className="text-[11px]"><span className="text-red-400">F0 ARMADO:</span> <span className="text-red-300">+20% CAPE Y +15% CPI — DETIENE COMPRAS</span></div><div className="text-[11px] text-slate-400 border-t border-slate-700 pt-1.5 mt-1 leading-tight"><span className="text-orange-300">G1 30%:</span> ΔFED&gt;0 O 10Y&gt;SMA200×1.10 <span className="text-slate-500">·</span> <span className="text-red-300">G2 70%:</span> ΔFED≥+0.50% & CPI&gt;SMA36M×1.30 <span className="text-slate-500">·</span> <span className="text-sky-300">G3 35%:</span> CPI 3M↓ O PAUSA 2M <span className="text-slate-500">·</span> <span className="text-emerald-300">G4 65%:</span> (CPI&lt;1.05 O FED&lt;0) & SMA50</div><div className="text-[11px]"><span className="text-emerald-400">SANACIÓN:</span> <span className="text-emerald-300">TRAS G1 (70/30) SIN G2 + CPI&lt;1.05 & SMA50 → 100%</span></div></div>):(<div className="space-y-1.5 text-left uppercase tracking-wide leading-tight text-slate-200"><div className="font-semibold text-emerald-400">📈 MÚLTIPLOS ALTOS OFF — DESARMADO <span className="text-emerald-400" style={{fontSize:'11px'}}>· BOSQUE NO SECO</span></div><div>CAPE <span className={capeOk?"text-red-300":"text-emerald-300 font-bold"}>{fmt(c)}</span> <span className="text-slate-400">{capeOk?">":"≤"}</span> UMBRAL <span className="text-slate-200 font-bold">{fmt(th)}</span> <span className="text-slate-400">(=SMA</span> <span className="text-slate-300">{fmt(s)}</span><span className="text-slate-400">×1.20)</span> <span className="text-slate-400">·</span> CORE CPI <span className={cpiOk?"text-red-300":"text-emerald-300 font-bold"}>{fmt(cpi)}</span> <span className="text-slate-400">{cpiOk?">":"≤"}</span> <span className="text-slate-200">{fmt(cpiThr)}</span></div><div className="text-[11px]"><span className="text-emerald-400">ESTADO:</span> <span className="text-emerald-300">DESARMADO — {!capeOk && !cpiOk ? "CAPE Y CPI DENTRO DE MEDIA" : !capeOk ? "CAPE DENTRO DE MEDIA" : "CPI DENTRO DE MEDIA"} — F0 NO ARMADO</span></div><div className="text-[11px] text-slate-500">F0: CAPE &gt; SMA36M×1.20 AND CPI &gt; SMA12M×1.15</div></div>); showTip(e,node); }} onMouseLeave={hideTip} onMouseMove={(e)=>{ const r=(e.currentTarget as HTMLElement).getBoundingClientRect(); setTableTip(prev=> prev? {...prev, x:r.left+r.width/2, y:r.top}:prev); }}>{d1.active?'📈':'⚪'}</span></td>
        ...
        <div className="text-[11px] text-slate-500 mt-2">Virtualizado · {sorted.length.toLocaleString('en-US')} filas visibles de {rowsView.length.toLocaleString('en-US')} totales · {virtualItems.length} renderizadas · 📈 CAPE&gt;1.20 & CPI&gt;1.15 · 🏦 Z&gt;2.0 (6M) · 😴 HY&lt;P20 — filtro Año para saltar rápido.</div>
      </div>

      <RegimesDocsTable />
      {tableTip && (
        <div className="fixed z-[100] pointer-events-none bg-slate-900 border border-slate-600 rounded-xl shadow-2xl p-3.5 text-xs max-w-[380px] ring-1 ring-white/10 text-slate-200" style={{ left: Math.min(Math.max(8, tableTip.x), typeof window !== 'undefined' ? window.innerWidth - 190 : 800), top: Math.max(8, tableTip.y - 8), transform: 'translate(-50%, -100%)' }}>
          {tableTip.node}
        </div>
      )}
