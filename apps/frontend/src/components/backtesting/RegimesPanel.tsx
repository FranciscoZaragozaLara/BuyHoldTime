...
export function RegimesPanel() {
  ...
  return (
    <div className="space-y-6">
                ...
                return (
                  <tr key={r.date} ref={isLast? observerRef : null} className="hover:bg-slate-800/40 border-t border-slate-800">
                    ...
                    {(() => {
                      ...
                      return (<>
                    <td className="p-2 text-center"><span className={icon(d1.active,'bg-amber-500/20 text-amber-300 border-amber-500/30','bg-slate-800 text-slate-500 border-slate-700')} onMouseEnter={(e)=>{ const c=d1.cape, s=d1.sma, th=d1.thr; const isRed = d1.active; const node=(<div className="space-y-1 text-left"><div className={"font-semibold "+(isRed?"text-red-400":"text-emerald-400")}>{isRed?"📈 Múltiplos Altos ON ✓ — DISPARADO":"📈 Múltiplos Altos OFF — Desarmado"} <span className={isRed?"text-red-400":"text-emerald-400"} style={{fontSize:'11px'}}>{isRed?"· supera umbral":"· por debajo de umbral"}</span></div><div>CAPE <span className={isRed?"text-red-400 font-bold":"text-emerald-400 font-bold"}>{fmt(c)}</span> {isRed?">":"≤"} umbral <span className="text-slate-300">{fmt(th)}</span> (=SMA <span className="text-slate-400">{fmt(s)}</span>×1.12)</div><div className="text-[11px]"><span className="text-slate-500">Estado:</span> <span className={isRed?"text-red-300":"text-emerald-300"}>{isRed?"Supera umbral — riesgo":"Desarmado — dentro de media"}</span></div></div>); showTip(e,node); }} onMouseLeave={hideTip} onMouseMove={(e)=>{ const c=d1.cape, s=d1.sma, th=d1.thr; const isRed = d1.active; const node=(<div className="space-y-1 text-left"><div className={"font-semibold "+(isRed?"text-red-400":"text-emerald-400")}>{isRed?"📈 Múltiplos Altos ON ✓ — DISPARADO":"📈 Múltiplos Altos OFF — Desarmado"}</div><div>CAPE <span className={isRed?"text-red-400 font-bold":"text-emerald-400 font-bold"}>{fmt(c)}</span> {isRed?">":"≤"} umbral <span className="text-slate-300">{fmt(th)}</span></div></div>); showTip(e,node); }}>{d1.active?'📈':'⚪'}</span></td>
                    <td className="p-2 text-center"><span className={icon(d2.active,'bg-sky-500/20 text-sky-300 border-sky-500/30','bg-slate-800 text-slate-500 border-slate-700')} onMouseEnter={(e)=>{ const isArmed=d2.active; const isDesarme=d2.z!=null && d2.z<0; const node=isArmed?(<div className="space-y-1 text-left"><div className="font-semibold text-sky-400">🏦 Alta Deuda ON ✓ — ARMADO (Fase 1) <span className="text-sky-400" style={{fontSize:'11px'}}>· memoria 6M</span></div><div>Z actual <span className={(d2.z??0)>2?"text-red-400 font-bold":"text-sky-400 font-bold"}>{fmt(d2.z,2)}</span> — armado por <span className="text-sky-400">Z&gt;2.0 en {(d2 as any).armedMonth ?? 'últimos 6M'}</span></div><div className="font-mono text-[11px]">Z=(<span className="text-slate-300">{fmt(d2.mg)}</span>−<span className="text-slate-400">{fmt(d2.mean)}</span>)/<span className="text-slate-400">{fmt(d2.std)}</span>=<span className="text-sky-400">{fmt(d2.z,2)}</span> <span className="text-sky-400">— persiste hasta Z&lt;0</span></div></div>):(<div className="space-y-1 text-left"><div className="font-semibold text-emerald-400">🏦 Alta Deuda OFF — {isDesarme?'Desarmado (Fase 4)':'No armado'} <span className="text-emerald-400" style={{fontSize:'11px'}}>{isDesarme?'· Z<0 purga completa':'· sin Z>2.0 en 6M'}</span></div><div>Z actual <span className="text-emerald-400 font-bold">{fmt(d2.z,2)}</span> <span className="text-slate-500">≤ 2.0</span></div><div className="text-[11px]"><span className="text-slate-500">Estado:</span> <span className="text-emerald-300">{isDesarme?'Desarmado — Z<0 desactiva':'No armado — sin peligro sistémico'}</span></div></div>); showTip(e,node); }} onMouseLeave={hideTip} onMouseMove={(e)=>{ const isArmed=d2.active; const node=isArmed?(<div className="space-y-1 text-left"><div className="text-sky-400">ARMADO</div></div>):(<div className="space-y-1 text-left"><div className="text-emerald-400">OFF</div></div>); showTip(e,node); }}>{d2.active?'🏦':'⚪'}</span></td>
                    <td className="p-2 text-center"><span className={icon(d3.active,'bg-violet-500/20 text-violet-300 border-violet-500/30','bg-slate-800 text-slate-500 border-slate-700')} onMouseEnter={(e)=>{ const isRed=d3.active; const node=isRed?(<div className="space-y-1 text-left"><div className="font-semibold text-red-400">😴 Complacencia ON ✓ — DISPARADO</div><div>HY OAS <span className="text-red-400 font-bold">{fmt(d3.hy)}%</span> · P20 <span className="text-slate-400">{fmt(d3.p20)}%</span></div></div>):(<div className="space-y-1 text-left"><div className="font-semibold text-emerald-400">😴 Complacencia OFF — Desarmado</div><div>HY OAS <span className="text-emerald-400">{fmt(d3.hy)}%</span> · P20 <span className="text-slate-400">{fmt(d3.p20)}%</span></div><div className="text-[11px]"><span className="text-slate-500">Estado:</span> <span className="text-emerald-300">Desarmado — sin complacencia</span></div></div>); showTip(e,node); }} onMouseLeave={hideTip} onMouseMove={(e)=>{ const isR=d3.active; showTip(e, isR?(<div className="text-red-400">ON</div>):(<div className="text-emerald-400">OFF</div>)); }}>{d3.active?'😴':'⚪'}</span></td>
                    </>); })()}
                    ...
                    <td className={`p-2 text-right font-mono font-semibold border rounded ${mgZColor} cursor-help`} onMouseEnter={(e)=>{ const n= mgZ!=null ? (detRow.r2.active ? (<div className="space-y-1 text-left"><div className="font-semibold text-sky-400">ARMADO Fase 1 — Z actual <span className="text-sky-400">{mgZ.toFixed(2)}</span></div><div className="font-mono text-[11px]">Z=(<span className="text-slate-300">{mg?.toFixed(2)}</span>−<span className="text-slate-400">{detRow.r2.mean?.toFixed(2) ?? '—'}</span>)/<span className="text-slate-400">{detRow.r2.std?.toFixed(2) ?? '—'}</span>=<span className="text-sky-400">{mgZ.toFixed(2)}</span> <span className="text-sky-400">— armado por Z&gt;2.0 en {(detRow.r2 as any).armedMonth ?? 'últimos 6M'} — persiste hasta Z&lt;0</span></div></div>) : (<div className="space-y-1 text-left"><div className="font-semibold text-emerald-400">NO ARMADO — Z actual <span className="text-emerald-400">{mgZ.toFixed(2)}</span></div><div className="font-mono text-[11px]">Z=(<span className="text-slate-300">{mg?.toFixed(2)}</span>−<span className="text-slate-400">{detRow.r2.mean?.toFixed(2) ?? '—'}</span>)/<span className="text-slate-400">{detRow.r2.std?.toFixed(2) ?? '—'}</span>=<span className="text-emerald-400">{mgZ.toFixed(2)}</span> <span className="text-slate-500">· ningún Z&gt;2.0 en 6M</span></div><div className="text-[11px]"><span className="text-slate-500">Estado:</span> <span className="text-emerald-300">Desarmado — sin peligro sistémico</span></div></div>)) : (<div className="text-slate-400">Z sin ventana</div>); const r=(e.currentTarget as HTMLElement).getBoundingClientRect(); setTableTip({x:r.left+r.width/2, y:r.top, node:n}); }} onMouseLeave={()=>setTableTip(null)} onMouseMove={(e)=>{ const n= mgZ!=null ? (detRow.r2.active ? (<div className="text-sky-400">ARMADO {mgZ.toFixed(2)}</div>) : (<div className="text-emerald-400">NO ARMADO {mgZ.toFixed(2)}</div>)) : (<div>—</div>); const r=(e.currentTarget as HTMLElement).getBoundingClientRect(); setTableTip({x:r.left+r.width/2, y:r.top, node:n}); }}>{mgZ!=null? mgZ.toFixed(2):'—'}</td>
      ...
      <RegimesDocsTable />
      {tableTip && (
        <div className="fixed z-[100] pointer-events-none bg-slate-800 border border-slate-600 rounded-xl shadow-2xl p-3.5 text-xs max-w-[380px] ring-1 ring-white/10" style={{ left: Math.min(Math.max(8, tableTip.x), typeof window !== 'undefined' ? window.innerWidth - 190 : 800), top: Math.max(8, tableTip.y - 8), transform: 'translate(-50%, -100%)' }}>
          {tableTip.node}
        </div>
      )}
