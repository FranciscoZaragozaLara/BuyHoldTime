    ...
    <div className="flex flex-col gap-8 w-full">
      ...
      <div className="flex justify-between items-center px-2">
        <p className="text-xs text-slate-400">
          {locale === 'es' ? 'Mostrando' : 'Showing'} <strong className="text-slate-100">{filteredTickers.length}</strong> {locale === 'es' ? 'acciones' : 'tickers'}
        </p>
      </div>
      ...
      {filteredTickers.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-16 rounded-2xl border border-dashed border-slate-800 bg-slate-950/20 text-center gap-3">
          ...
          <p className="text-sm font-semibold text-slate-400">
            {locale === 'es' ? 'No se encontraron acciones coincidentes.' : 'No matching tickers found.'}
          </p>
        </div>
      ) : viewMode === 'grid' ? (
        ...
        /* Comprehensive Table View with all financial metrics */
