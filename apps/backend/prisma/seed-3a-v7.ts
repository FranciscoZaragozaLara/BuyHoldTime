...
async function m(){
  const code="SCHILLER_TQQQ_3A_RISK_D_V7";
  const strat=await p.btStrategy.upsert({where:{code}, update:{name:"Schiller 3A RISK D V7 sweetspot (mean 3y -0.02)", description:"3A tune 1.08 3% 1.13 6% 1.18 10% 1.23 12% f0.74 supera V6 5A"}, create:{code, name:"Schiller 3A RISK D V7 sweetspot (mean 3y -0.02)", description:"3A V7", paramsSchema:{lookback:36}}});
  const base="/Users/zilphfanel/Documents/AgyApps/BackTesting";
  const met=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v7.json`,"utf8"));
  ...
  const tr=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v7_trades.json`,"utf8"));
  const mo=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v7_allocations_monthly.json`,"utf8"));
  const eq=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v7_equity_full.json`,"utf8"));
  ...
  console.log("seeded 3A V7",run.id,met.final_value);
}
