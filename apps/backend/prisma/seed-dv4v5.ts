...
async function m(){
  const code="SCHILLER_TQQQ_5A_RISK_D_V4_V5";
  const strat=await p.btStrategy.upsert({where:{code}, update:{name:"Schiller 5A RISK D V4 V5 (cash 5% desde 1.15 hasta 12% en 1.25)", description:"V5 cash 1.15->5% 1.20->10% 1.25->12%"}, create:{code, name:"Schiller 5A RISK D V4 V5 (cash 5% desde 1.15 hasta 12% en 1.25)", description:"V5", paramsSchema:{v5:true}}});
  const base="/Users/zilphfanel/Documents/AgyApps/BackTesting";
  const met=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4_v5.json`,"utf8"));
  ...
  const tr=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4_v5_trades.json`,"utf8"));
  const mo=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4_v5_allocations_monthly.json`,"utf8"));
  const eq=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4_v5_equity_full.json`,"utf8"));
  ...
  console.log("seeded V5",run.id,met.final_value);
}
