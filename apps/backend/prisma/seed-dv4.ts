...
async function m(){
  const code="SCHILLER_TQQQ_5A_RISK_D_V4";
  const strat=await p.btStrategy.upsert({where:{code}, update:{name:"Schiller 5A RISK D V4 (cash 5/8/12/16% + lump <1.0)", description:"Dv4 cash reserve by ratio"}, create:{code, name:"Schiller 5A RISK D V4 (cash 5/8/12/16% + lump <1.0)", description:"Dv4", paramsSchema:{v4:true}}});
  const base="/Users/zilphfanel/Documents/AgyApps/BackTesting";
  const met=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4.json`,"utf8"));
  ...
  const tr=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4_trades.json`,"utf8"));
  const mo=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4_allocations_monthly.json`,"utf8"));
  const eq=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v4_equity_full.json`,"utf8"));
  ...
  console.log("seeded Dv4",run.id,met.final_value);
}
