...
async function runBacktest(startDate: string): Promise<any> {
  const base = '/Users/zilphfanel/Documents/AgyApps/BackTesting';
  return new Promise((resolve, reject) => {
    const proc = spawn('python3', ['run_risk_d_3a_v8.py'], { cwd: base, env: { ...process.env, START_DATE: startDate } });
    let stderr = '';
    proc.stderr.on('data', d => stderr += d.toString());
    proc.on('close', code => code === 0 ? resolve(null) : reject(new Error(`Failed ${startDate} code ${code} ${stderr.slice(0,500)}`)));
  });
}
...
async function main() {
  ...
  if (!strat) throw new Error(`Strategy ${code} not found`);
  ...
  for (const d of dates) {
    ...
    if (ds < inception) {
      console.log(`Skip ${ds} ${d.descriptor} - inception ${inception} (unable)`);
      continue;
    }
    const existing = await prisma.btBacktestRun.findFirst({ where: { strategyId: strat.id, startDate: new Date(ds) } });
    if (existing) {
      console.log(`Exists ${ds} ${d.descriptor} skip`);
      continue;
    }
    console.log(`Running ${ds} ${d.descriptor}...`);
    ...
    const run = await prisma.btBacktestRun.create({
      data: { strategyId: strat.id, startDate: new Date(ds), endDate: new Date(eq[eq.length-1]?.date || '2026-08-12'), initialCash: 100000, commission: 0.0005, paramsUsed: met },
    });
    ...
    for (let i = 0; i < eq.length; i += 500) await prisma.btEquityCurve.createMany({ data: eq.slice(i,i+500).map((e:any)=>({ runId: run.id, date: new Date(e.date), portfolioValue: e.portfolio_value ?? e.portfolioValue })), skipDuplicates: true });
    console.log(`Seeded ${ds} final ${met.final_value} CAGR ${(met.cagr*100).toFixed(2)}%`);
  }
  console.log('Done');
}
