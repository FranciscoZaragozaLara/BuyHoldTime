...
async function seed(code:string, name:string, file:string, start:string){
  ...
  if(allocFileToUse){
    ...
    const toStore = mo.length>500 ? mo.filter((_:any,i:number)=> i%5===0).slice(0,2000) : mo;
...
async function m(){
  await seed("SPY_SGOV_V9_2010","SPY SGOV V9 3y 2010 cap 20%","result_spy_sgov_v9.json","2010-09-09");
  await seed("VOO_SGOV_V9_2010","VOO SGOV V9 3y 2010 cap 20%","result_voo_sgov_v9.json","2010-09-09");
  await seed("SCHILLER_TQQQ_3A_RISK_D_V9","Schiller 3A RISK D V9 cap 20%","result_schiller_3a_risk_d_v9.json","2010-02-11");
}
