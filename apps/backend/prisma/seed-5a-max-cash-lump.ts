...
async function main(){
  const code='SCHILLER_TQQQ_5A_MAX_CASH';
  ...
  const metrics = JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_max_cash.json`,'utf8'));
  const trades = JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_max_cash_trades.json`,'utf8'));
  const monthly = JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_max_cash_allocations_monthly.json`,'utf8'));
  const equity = JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_max_cash_equity_full.json`,'utf8'));
  ...
  for(let i=0;i<trades.length;i+=200){
    ...
    await prisma.btTrade.createMany({ data: chunk.map((t:any)=>({
      runId: run.id,
      ticker: t.ticker,
      side: t.side,
      size: t.size,
  ...
  console.log('trades inserted', trades.length);
  ...
  console.log('allocs inserted', monthly.length);
  ...
  console.log('equity inserted', equity.length);
