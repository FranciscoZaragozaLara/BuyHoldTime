...
async function m(){
  const code="SCHILLER_TQQQ_3A_RISK_D_V6";
  const strat=await p.btStrategy.upsert({where:{code}, update:{name:"Schiller 3A RISK D V6 (mean 3y) clon V6", description:"Clon V6 con mean CAPE 3y (36m) para adaptarse más rápido"}, create:{code, name:"Schiller 3A RISK D V6 (mean 3y) clon V6", description:"3A V6", paramsSchema:{lookback:36}}});
  const base="/Users/zilphfanel/Documents/AgyApps/BackTesting";
  const met=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v6.json`,"utf8"));
  ...
  const tr=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v6_trades.json`,"utf8"));
  const mo=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v6_allocations_monthly.json`,"utf8"));
  const eq=JSON.parse(fs.readFileSync(`${base}/result_schiller_3a_risk_d_v6_equity_full.json`,"utf8"));
  ...
  console.log("seeded 3A V6",run.id,met.final_value);
}
