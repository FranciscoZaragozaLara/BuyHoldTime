...
async function m(){
  const code="SCHILLER_TQQQ_5A_RISK_D_V6";
  const strat=await p.btStrategy.upsert({where:{code}, update:{name:"Schiller 5A RISK D V6 sweetspot (1.10 3% 1.15 6% 1.20 10% 1.25 12% f0.74)", description:"Tuned sobre V4/V5 f0.74 ladder ligero"}, create:{code, name:"Schiller 5A RISK D V6 sweetspot (1.10 3% 1.15 6% 1.20 10% 1.25 12% f0.74)", description:"V6", paramsSchema:{v6:true}}});
  const base="/Users/zilphfanel/Documents/AgyApps/BackTesting";
  const met=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v6.json`,"utf8"));
  ...
  const tr=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v6_trades.json`,"utf8"));
  const mo=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v6_allocations_monthly.json`,"utf8"));
  const eq=JSON.parse(fs.readFileSync(`${base}/result_schiller_5a_risk_d_v6_equity_full.json`,"utf8"));
  ...
  console.log("seeded V6",run.id,met.final_value);
}
