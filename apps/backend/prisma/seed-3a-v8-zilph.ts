...
async function m(){
  ...
  const strat=await p.btStrategy.upsert({where:{code}, update:{name:"Schiller 3A RISK D V8 ZILPH VIX>25 0.95", description:"Zilph VIX>25 RSI38 c2m0.65 0.95 panic + post-panic 90d 0.90 + bull-catchup 2023"}, create:{code, name:"Schiller 3A RISK D V8 ZILPH VIX>25 0.95", description:"Zilph 0.95", paramsSchema:{zilph:true}}});
  ...
  console.log("seeded ZILPH 0.95",run.id,met.final_value);
}
